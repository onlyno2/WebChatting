<template>
  <v-col lg="4" md="6">
    ...
    >
      <v-card-title>
        <span class="title font-weight-light text-truncate"><p class="font-weight-bold mb-0">{{ post.title }}</p></span>
      </v-card-title>

      <v-card-text class="font-weight-bold text-left overflow-hidden" style="height: 72px">
        <p class="font-weight-light">{{ post.content }}</p>
      </v-card-text>

      <v-card-actions>
        <v-list-item class="grow">
          <v-list-item-avatar color="grey darken-3" @click="dialog = true" >
            <v-img
              class="elevation-6"
          ...
          <v-list-item-content>
            <v-list-item-title @click="dialog = true" class="text-left">{{ post.name }}</v-list-item-title>
          </v-list-item-content>

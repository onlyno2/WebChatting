  ...
  <v-card>
    <v-container>
      ...
      <v-row align="center" justify="center">
        <v-col md="4">
          <v-avatar size="200">
            <img v-bind:src="userPicture" alt="John" />
          </v-avatar>
        </v-col>
        <v-col md="4">
          <p>{{ user.name }}</p>
      ...
      <v-row align="center" justify="center">
        <v-col md="4">
          <p class="font-weight-bold mx-auto">{{ $t('dislikeCount') }}</p>
          <v-avatar color="blue lighten-3" size="200">
            <p class="display-3">{{ user.like }}</p>
          </v-avatar>
        </v-col>
        <v-col md="4">
          <p class="font-weight-bold mx-auto">{{ $t('likeCount') }}</p>
          <v-avatar color="red lighten-3" size="200">
            <p class="display-3">{{ user.dislike }}</p>
          </v-avatar>
        </v-col>
      </v-row>
      <v-row class="mt-4" align="center" justify="center" v-if="loggedIn">
        <v-col md="4">
          <v-btn block :to="{ name: 'editProfile', params: { id: editId } }">{{
            $t('editProfile')
          }}</v-btn>
        </v-col>
        <v-col md="4">
          <v-btn block color="success" to="/createpost">{{
            $t('createPost')
          }}</v-btn>
        </v-col>
      </v-row>
